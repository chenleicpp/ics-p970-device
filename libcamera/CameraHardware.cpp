...
/* 29/12/10 : preview/picture size validation logic */
	...
	const char CameraHardware::supportedPreviewSizes_bfc[] =
	    "1280x720,800x480,720x480,640x480,352x288";
	...
	bool CameraHardware::validateSize(size_t width, size_t height,
	...
	int CameraHardware::previewThread() {
		...
		if (!previewStopped) {
			...
			if (mNativeWindow != NULL) {
				...
				mNativeWindow->lock_buffer(mNativeWindow,
							   (buffer_handle_t *)
							   hndl2hndl);
				...
				if (0 ==
				    mapper.lock((buffer_handle_t) * hndl2hndl,
						GRALLOC_USAGE_SW_WRITE_OFTEN,
						bounds, &dst)) {
					...
					if (mRecordingEnabled) {
						tempbuf =
						    mCamera->
						    GrabRecordFrame(index);
						nsecs_t timeStamp =
						    systemTime
						    (SYSTEM_TIME_MONOTONIC);
						memcpy(mRecordHeap[index]->data,
						       tempbuf,
						       mRecordFramesize);
						...
						// Get preview frame
						tempbuf =
						    mCamera->GrabPreviewFrame();
					}
					yuv422_to_YV12((unsigned char *)tempbuf,
						       (unsigned char *)dst,
						       width, height);
					mapper.unlock((buffer_handle_t) *
						      hndl2hndl);

					mNativeWindow->
					    enqueue_buffer(mNativeWindow,
							   (buffer_handle_t *)
							   hndl2hndl);
