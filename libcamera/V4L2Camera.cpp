...
//chenlei : use imx072 format
#define DEF_PIX_FMT             V4L2_PIX_FMT_SRGGB10
...
#define CAMERA_FF 1		//Front Camera
	...
	if (!m_flag_init) {
		do {
			if ((camHandle = open(device, O_RDWR)) == -1) {
				LOGE("ERROR opening V4L interface: %s",
					 strerror(errno));
			...
			if (version >= KERNEL_VERSION(2, 6, 37)) {
				...
				ret =
					ioctl(ccdc_fd, VIDIOC_SUBDEV_S_FMT,
					  &fmt);
				if (ret < 0) {
			...
			ret =
				ioctl(camHandle, VIDIOC_QUERYCAP,
				  &videoIn->cap);
			if (ret < 0) {
...
int V4L2Camera::Open_media_device(const char *device) {
	...
	do {
		mediaIn->entity[index].id =
			index | MEDIA_ENTITY_ID_FLAG_NEXT;
		ret =
			ioctl(mediaIn->media_fd, MEDIA_IOC_ENUM_ENTITIES,
			  &mediaIn->entity[index]);
		if (ret < 0) {
			break;
		} else {
			if (!strcmp
				(mediaIn->entity[index].name,
				 ENTITY_VIDEO_CCDC_OUT_NAME))
				mediaIn->video =
					mediaIn->entity[index].id;
			else if (!strcmp
				 (mediaIn->entity[index].name,
	...
	for (index = 0; index < mediaIn->num_entities; index++) {
		...
		links.pads =
			...
			malloc((sizeof(struct media_pad_desc)) *
			   (mediaIn->entity[index].pads));
		links.links =
			(struct media_link_desc *)
			malloc((sizeof(struct media_link_desc)) *
			   mediaIn->entity[index].links);
		ret =
			ioctl(mediaIn->media_fd, MEDIA_IOC_ENUM_LINKS,
			  &links);
		if (ret < 0) {
			...
			if (mediaIn->entity[index].pads)
				LOGD("pads for entity %d=",
					 mediaIn->entity[index].id);
			for (i = 0; i < mediaIn->entity[index].pads;
				 i++) {
				LOGD("(%d %s) ", links.pads->index,
					 (links.pads->
					  flags & MEDIA_PAD_FLAG_INPUT) ?
					 "INPUT" : "OUTPUT");
				links.pads++;
			}
			for (i = 0; i < mediaIn->entity[index].links;
				 i++) {
				LOGD("[%d:%d]===>[%d:%d]",
...
int V4L2Camera::Configure(int width, int height, int pixelformat,
			  int fps, int cam_mode) {
	int ret = 0;
	...
	if (version >= KERNEL_VERSION(2, 6, 37)) {
		...
		videoIn->framesizeIn =
			((IMG_WIDTH_VGA * IMG_HEIGHT_VGA) << 1);
	...
	do {
		...
		if (ret < 0) {
			LOGE("Open: VIDIOC_S_FMT Failed: %s",
				 strerror(errno));
			break;
		}
		LOGD("CameraConfigure PreviewFormat: w=%d h=%d",
...
int V4L2Camera::BufferMap(int cam_mode) {
	...
	for (int i = 0; i < mMaxBuffers; i++) {
		...
		if (ret < 0) {
			LOGE("Init: Unable to query buffer (%s)",
				 strerror(errno));
			return ret;
		}
		...
		if (videoIn->mem[i] == MAP_FAILED) {
			LOGE("Init: Unable to map buffer (%s)",
				 strerror(errno));
			return -1;
		}
...
void V4L2Camera::Uninit(int cam_mode) {
	...
	if (m_flag_init) {
		...
		/* Unmap buffers */
		for (int i = 0; i < mMaxBuffers; i++)
			if (munmap(videoIn->mem[i], videoIn->buf.length)
				< 0)
...
void V4L2Camera::GrabRawFrame(void *previewBuffer, unsigned int width,
				  unsigned int height) {
	int ret = 0;
	int DQcount = 0;
...
#ifdef _OMAP_RESIZER_
		ret =
			OMAPResizerConvert(videoIn->resizeHandle,
					   videoIn->mem[videoIn->buf.index],
					   videoIn->format.fmt.pix.height,
					   videoIn->format.fmt.pix.width,
					   previewBuffer, height, width);
		if (ret < 0)
			LOGE("Resize operation:%d", ret);
#endif //_OMAP_RESIZER_
	} else {
		memcpy(previewBuffer, videoIn->mem[videoIn->buf.index],
			   (size_t) videoIn->buf.bytesused);
	}
...
unsigned long V4L2Camera::savePicture(unsigned char *inputBuffer,
			 unsigned char *&output) {
	unsigned long fileSize = 100;
	int ret;
	fileSize =
		saveYUYVtoJPEG(inputBuffer, videoIn->width, videoIn->height,
			   output, 95);
	return fileSize;
}

camera_memory_t *V4L2Camera::GrabJpegFrame(camera_request_memory mRequestMemory,
		  unsigned long &mfilesize, bool IsFrontCam) {
	...
	do {
		...
		if (IsFrontCam) {
			...
			fileSize =
				savePicture((unsigned char *)videoIn->
					mem[videoIn->buf.index],
					outputBuffer);
		}
		...
		if (IsFrontCam) {
			...
			picture = mRequestMemory(-1, fileSize, 1, NULL);
			picture->data =
				(unsigned char *)videoIn->mem[videoIn->buf.
							  index];
			mfilesize = fileSize;
		}
		break;
...
camera_memory_t *V4L2Camera::CreateJpegFromBuffer(void *rawBuffer,
	...
	do {
		...
		fileSize =
			savePicture((unsigned char *)videoIn->
				mem[videoIn->buf.index], outputBuffer);
...
unsigned long V4L2Camera::saveYUYVtoJPEG(unsigned char *inputBuffer,
...
// p970 dont have kernel ioctl in driver
// so we need not it
// Autofocus Functions
...
/**
	...
	if ((count >= FIRST_AF_SEARCH_COUNT) || (ret != AF_SUCCESS)) {
		LOGV("%s : 1st AF timed out, failed, or was canceled",
			 __func__);
		af_result = 0;
	...
	af_result = 1;
	LOGV("%s : AF was successful, returning %d", __func__,
		 af_result);
...
int V4L2Camera::setZoom(int zoom_level) {
	...
	if (zoom_level < ZOOM_LEVEL_0 || ZOOM_LEVEL_MAX <= zoom_level) {
		LOGE("ERR(%s):Invalid zoom_level (%d)", __func__,
			 zoom_level);
		return -1;
	}
	...
	if (videoIn->isStreaming) {
		...
		if (mZoomLevel != zoom_level) {
			...
			if (v4l2_s_ctrl
				(camHandle, V4L2_CID_ZOOM,
				 zoom_level) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_ZOOM",
					 __func__);
				return -1;
			}
		}
...
int V4L2Camera::setWhiteBalance(int white_balance) {
	...
	if (white_balance <= WHITE_BALANCE_BASE
		...
		LOGE("ERR(%s):Invalid white_balance(%d)", __func__,
			 white_balance);
		return -1;
	}
	...
	if (videoIn->isStreaming) {
		if (mWhiteBalance != white_balance) {
			...
			if (v4l2_s_ctrl
				(camHandle, V4L2_CID_WB,
				 white_balance) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_WB",
					 __func__);
				return -1;
			}
		}
...
int V4L2Camera::setSceneMode(int scene_mode) {
	...
	if (scene_mode <= SCENE_MODE_BASE
		|| SCENE_MODE_MAX <= scene_mode) {
		LOGE("ERR(%s):Invalid scene_mode (%d)", __func__,
			 scene_mode);
		return -1;
	}
	...
	if (videoIn->isStreaming) {
		...
		if (mSceneMode != scene_mode) {
			...
			if (v4l2_s_ctrl
				(camHandle, V4L2_CID_SCENE,
				 scene_mode) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_SCENE",
					 __func__);
				return -1;
			}
		}
...
int V4L2Camera::setFocusMode(int focus_mode) {
	...
	if (FOCUS_MODE_MAX <= focus_mode) {
		LOGE("ERR(%s):Invalid focus_mode (%d)", __func__,
			 focus_mode);
		return -1;
	}
...
int V4L2Camera::setAEAWBLockUnlock(int ae_lockunlock,
				   int awb_lockunlock) {
...
int V4L2Camera::setExifOrientationInfo(int orientationInfo) {
	...
	if (orientationInfo < 0) {
		LOGE("ERR(%s):Invalid orientationInfo (%d)", __func__,
			 orientationInfo);
		return -1;
	}
...
int V4L2Camera::setISO(int iso_value) {
	if (iso_value < ISO_AUTO || ISO_MAX <= iso_value) {
		LOGE("ERR(%s):Invalid iso_value (%d)", __func__,
			 iso_value);
		return -1;
	}
	...
	if (videoIn->isStreaming) {
		if (mISO != iso_value) {
			mISO = iso_value;
			if (v4l2_s_ctrl
				(camHandle, V4L2_CID_ISO, iso_value) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_ISO",
					 __func__);
				return -1;
			}
		}
...
int V4L2Camera::setMetering(int metering_value) {
	...
	if (metering_value <= METERING_BASE
		|| METERING_MAX <= metering_value) {
		LOGE("ERR(%s):Invalid metering_value (%d)", __func__,
			 metering_value);
		return -1;
	}
...
int V4L2Camera::setContrast(int contrast_value) {
	...
	if (contrast_value < CONTRAST_MINUS_2
		|| CONTRAST_MAX <= contrast_value) {
		LOGE("ERR(%s):Invalid contrast_value (%d)", __func__,
			 contrast_value);
		return -1;
	}
...
int V4L2Camera::setSharpness(int sharpness_value) {
	...
	if (sharpness_value < SHARPNESS_MINUS_2
		...
		LOGE("ERR(%s):Invalid sharpness_value (%d)", __func__,
			 sharpness_value);
		return -1;
	}
	if (videoIn->isStreaming) {
...
int V4L2Camera::setBrightness(int brightness) {
	...
	if (brightness < EV_MINUS_4 || EV_PLUS_4 < brightness) {
		LOGE("ERR(%s):Invalid brightness(%d)", __func__,
			 brightness);
		return -1;
	}
	if (videoIn->isStreaming) {
...
void V4L2Camera::convert(unsigned char *buf, unsigned char *rgb,
			 int width, int height) {
	int x, y, z = 0;
	int blocks;
