			  ...
			  camera_request_memory get_memory, void *user)
{
	...
	LOG_FUNCTION_NAME
	V4L2CameraHardware->setCallbacks(notify_cb, data_cb,
					 data_cb_timestamp, get_memory,
					 user);
}
...
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->startPreview();
}

void camera_stop_preview(struct camera_device *device)
{
	LOG_FUNCTION_NAME V4L2CameraHardware->stopPreview();
}

int camera_preview_enabled(struct camera_device *device)
{
	LOG_FUNCTION_NAME if (V4L2CameraHardware->previewEnabled()) {
...
{
	int rv = -EINVAL;
	LOG_FUNCTION_NAME return rv;
}

int camera_start_recording(struct camera_device *device)
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->startRecording();
}

void camera_stop_recording(struct camera_device *device)
{
	LOG_FUNCTION_NAME V4L2CameraHardware->stopRecording();
}

int camera_recording_enabled(struct camera_device *device)
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->recordingEnabled();
}
...
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->autoFocus();
}

int camera_cancel_auto_focus(struct camera_device *device)
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->cancelAutoFocus();
}

int camera_take_picture(struct camera_device *device)
{
	LOG_FUNCTION_NAME return V4L2CameraHardware->takePicture();
}

int camera_cancel_picture(struct camera_device *device)
{
	int rv = 0;		// -EINVAL;
	LOG_FUNCTION_NAME return V4L2CameraHardware->cancelPicture();
}

int camera_set_parameters(struct camera_device *device, const char *params)
{
	LOG_FUNCTION_NAME CameraParameters * camParams = new CameraParameters();
...
{
	LOG_FUNCTION_NAME V4L2CameraHardware->release();
}
...
{
	int ret = 0;
	LOG_FUNCTION_NAME delete V4L2CameraHardware;
		       ...
		       hw_device_t ** device)
{
	...
	LOG_FUNCTION_NAME LOGI("camera_device open");
	...
	if (name != NULL) {
		...
		camera_device =
		    (V4l2_camera_device_t *) malloc(sizeof(*camera_device));
		if (!camera_device) {
		...
		camera_ops =
		    (camera_device_ops_t *) malloc(sizeof(*camera_ops));
		if (!camera_ops) {
...
{
	LOG_FUNCTION_NAME int num_cameras = 2;	// MAX_CAMERAS_SUPPORTED;
	return num_cameras;
}

int camera_get_camera_info(int camera_id, struct camera_info *info)
{
	LOG_FUNCTION_NAME int rv = 0;
	...
	if (camera_id == 0) {
		...
		info->orientation = 90;//p970:fix front cam orientation
	}
