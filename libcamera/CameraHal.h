...
///Forward declarations
	...
	class CameraArea:public RefBase {
		...
		CameraArea(ssize_t top,
		    ...
		    mLeft(left),
		    mBottom(bottom), mRight(right), mWeight(weight) {
		} status_t transfrom(size_t width,
				     size_t height,
				     size_t & top,
				     size_t & left,
				     size_t & areaWidth, size_t & areaHeight);
		...
		static bool areAreasDifferent(Vector < sp < CameraArea > >&,
					      Vector < sp < CameraArea > >&);
	...
	class CameraFrame {
		...
		//default contrustor
		 CameraFrame():mCookie(NULL),
		    ...
		    mOffset(0),
		    mAlignment(0),
		    mFd(0), mLength(0), mFrameMask(0), mQuirks(0) {

			mYuv[0] = NULL;
			mYuv[1] = NULL;
		}
		//copy constructor CameraFrame(const CameraFrame & frame):
		 mCookie(frame.mCookie),
		    ...
		    mFrameMask(frame.mFrameMask), mQuirks(frame.mQuirks) {
...
///@todo Rename this class to CameraEvent
	class CameraHalEvent {
		...
		//default contrustor
		 CameraHalEvent(): mCookie(NULL), mEventType(NO_EVENTS) {
		}
		//copy constructor CameraHalEvent(const CameraHalEvent & event):
		 mCookie(event.mCookie),
		    mEventType(event.mEventType), mEventData(event.mEventData) {
		};
  ...
  */
	class MessageNotifier {
		...
		virtual void enableMsgType(int32_t msgs,
					   frame_callback frameCb =
					   NULL, event_callback eventCb =
					   NULL, void *cookie = NULL) = 0;
  ...
  */
	class FrameNotifier:public MessageNotifier {
		...
		virtual void returnFrame(void *frameBuf,
					 CameraFrame::FrameType frameType) = 0;
  ...
  */
	class FrameProvider {
		...
		frame_callback mFrameCallback;
	      ...
	      public:
		 FrameProvider(FrameNotifier * fn, void *cookie,
			       frame_callback frameCallback)
		:mFrameNotifier(fn), mCookie(cookie),
  ...
  */
	class EventProvider {
		...
		event_callback mEventCallback;
	      ...
	      public:
		 EventProvider(MessageNotifier * mn, void *cookie,
			       event_callback eventCallback)
		:mEventNotifier(mn), mCookie(cookie),
...
/*
  ...
  */
	class BufferProvider {
	      public:
		virtual void *allocateBuffer(int width, int height,
...
/**
  ...
  */
	class AppCallbackNotifier:public ErrorNotifier, public virtual RefBase {
		...
		void setEventProvider(int32_t eventMask,
				      MessageNotifier * eventProvider);
		...
		status_t initSharedVideoBuffers(void *buffers,
						uint32_t * offsets, int fd,
						size_t length, size_t count,
						void *vidBufs);
		...
		void EncoderDoneCb(void *, void *, CameraFrame::FrameType type,
		...
		status_t dummyRaw();
		void copyAndSendPictureFrame(CameraFrame * frame,
					     int32_t msgType);
		void copyAndSendPreviewFrame(CameraFrame * frame,
					     int32_t msgType);
		...
		KeyedVector < uint32_t,
		    uint32_t > mVideoMetadataBufferMemoryMap;
		KeyedVector < uint32_t,
		    uint32_t > mVideoMetadataBufferReverseMap;
		...
		KeyedVector < unsigned int,
		    sp < MemoryHeapBase > >mSharedPreviewHeaps;
		KeyedVector < unsigned int,
		    sp < MemoryBase > >mSharedPreviewBuffers;
...
/**
  ...
  */
	...
	class CameraAdapter:public FrameNotifier, public virtual RefBase {
		...
		enum AdapterState {
			...
			LOADED_PREVIEW_STATE =
			    LOADED_PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			PREVIEW_STATE = PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			LOADED_CAPTURE_STATE =
			    LOADED_CAPTURE_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
			CAPTURE_STATE =
			    CAPTURE_ACTIVE | PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			BRACKETING_STATE =
			    BRACKETING_ACTIVE | CAPTURE_ACTIVE | PREVIEW_ACTIVE
			    | INTIALIZED_ACTIVE,
			AF_STATE =
			    AF_ACTIVE | PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			ZOOM_STATE =
			    ZOOM_ACTIVE | PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			VIDEO_STATE =
			    VIDEO_ACTIVE | PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			VIDEO_AF_STATE =
			    VIDEO_ACTIVE | AF_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
			VIDEO_ZOOM_STATE =
			    VIDEO_ACTIVE | ZOOM_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
			VIDEO_LOADED_CAPTURE_STATE =
			    VIDEO_ACTIVE | LOADED_CAPTURE_ACTIVE |
			    PREVIEW_ACTIVE | INTIALIZED_ACTIVE,
			VIDEO_CAPTURE_STATE =
			    VIDEO_ACTIVE | CAPTURE_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
			AF_ZOOM_STATE =
			    AF_ACTIVE | ZOOM_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
			BRACKETING_ZOOM_STATE =
			    BRACKETING_ACTIVE | ZOOM_ACTIVE | PREVIEW_ACTIVE |
			    INTIALIZED_ACTIVE,
		};
		...
		status_t flushBuffers() {
			return sendCommand(CameraAdapter::
					   CAMERA_PREVIEW_FLUSH_BUFFERS);
		}
		//Registers callback for returning image buffers back to CameraHAL
		    virtual int
		    registerImageReleaseCallback(release_image_buffers_callback
						 callback, void *user_data) = 0;

		//Registers callback, which signals a completed image capture
		virtual int
		    registerEndCaptureCallback(end_image_capture_callback
					       callback, void *user_data) = 0;

		//API to send a command to the camera
		virtual status_t sendCommand(CameraCommands operation,
					     int value1 = 0, int value2 =
					     0, int value3 = 0) = 0;
		...
		virtual void onOrientationEvent(uint32_t orientation,
						uint32_t tilt) = 0;
	...
	class DisplayAdapter:public BufferProvider, public virtual RefBase {
		...
		virtual int setErrorHandler(ErrorNotifier * errorNotifier) = 0;
		virtual int enableDisplay(int width, int height,
					  struct timeval *refTime =
					  NULL, S3DParameters * s3dParams =
					  NULL) = 0;
		...
		virtual int setSnapshotTimeRef(struct timeval *refTime = NULL) =
		    0;
#endif
...
/*--------------------Internal Member functions - Private---------------------------------*/
    ...
    /** Restart the preview with setParameter. */
		...
		status_t parseResolution(const char *resStr, int &width,
					 int &height);
    ...
    /** Allocate preview buffers */
		status_t allocPreviewBufs(int width, int height,
					  const char *previewFormat,
					  unsigned int bufferCount,
					  unsigned int &max_queueable);

    /** Allocate video buffers */
		status_t allocVideoBufs(uint32_t width, uint32_t height,
					uint32_t bufferCount);

    /** Allocate image capture buffers */
		status_t allocImageBufs(unsigned int width, unsigned int height,
					size_t length,
					const char *previewFormat,
					unsigned int bufferCount);
    ...
    /** Initialize default parameters */
		...
		void resetPreviewRes(CameraParameters * mParams, int width,
				     int height);
