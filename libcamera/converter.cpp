...
void
yuyv422_to_yuv420(unsigned char *bufsrc, unsigned char *bufdest, int width,
		  int height)
{
	...
	ptrdestcr2 =
	    bufdest + width * height + ((width * height) >> 2) + (width >> 1);
...
void
yuyv422_to_yuv420sp(unsigned char *bufsrc, unsigned char *bufdest, int width,
		    int height)
{
	unsigned char *ptrsrcy1, *ptrsrcy2;
	unsigned char *ptrsrcy3, *ptrsrcy4;
...
void
yuyv422_to_yuv422sp(unsigned char *bufsrc, unsigned char *bufdest, int width,
		    int height)
{
	unsigned char *ptrsrcy1, *ptrsrcy2;
	unsigned char *ptrsrcy3, *ptrsrcy4;
...
void convertYUYVtoRGB565(unsigned char *srcBuf, unsigned char *dstBuf,
			 int srcWidth, int srcHeight)
{
	...
	for (unsigned int y = 0; y < srcHeight; y += 2) {
		...
		for (unsigned int x = 0; x < (srcWidth * 2); x += 4) {
			...
			dstBufPointer[dst_cbcr_pos++] =
			    srcBufPointer[src_pos + 3];
			dstBufPointer[dst_cbcr_pos++] =
			    srcBufPointer[src_pos + 1];
		}
	}
